...
opt['batch_size'] = 8
opt['atWhichSave'] = [2, 70002, 100002, 150002, 200002] #[2,100002,150002,200002,250002,300002,350002,400002,450002,500002,550000, 600000,650002,700002,750000,800000,850002,900002,950000,1000000] # testing will be done at these iterations and corresponding model weights will be saved.
opt['iterations'] = 200005 #1000005 # The model will run for these many iterations.
...
# These are folders
save_weights = 'outputs/weights' # Model weights will be saved here.
save_images = 'outputs/images' # Restored images will be saved here.
save_csv_files = 'outputs/csv_files' # Other details such as loss value and learning rate will be saved in this file.
...
while iter_num<opt['iterations']:
    ...
    #np.savetxt(os.path.join(save_csv_files,'loss_curve{}.csv'.format(iter_num)),[p for p in zip(loss_iter_list,loss_list,iter_LR)],delimiter=',',fmt='%s')   
